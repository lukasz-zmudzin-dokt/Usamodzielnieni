...
import {render, waitForElement, fireEvent, queries} from '@testing-library/react';
...
describe('load cv data', () => {
        ...
        global.fetch = jest.fn().mockImplementation((input, init) => {
            return new Promise((resolve, reject) => {
                if (apiFail) {
                    ...
                    resolve({status: 200, json: () => {Promise.resolve(data); Promise.resolve(feedback)}});
                } else {
    ...
    it('should load correct data on personal tab', async () => {
        ...
        await expect(getByLabelText("Imię:").value).toBe(data.basic_info.first_name);
