...
describe("PersonalDataTab", () => {
  ...
  it("should render without crashing", () => {
    const { container } = render(<PersonalDataTab {...props} />);

    expect(container).toMatchSnapshot();
  });
  it("should not render when data is null", () => {
    props.data = null;
    const { queryByText } = render(<PersonalDataTab {...props} />);
  ...
  it("should call onChange function when text input value change", async () => {
    ...
    const { getByLabelText } = render(<PersonalDataTab {...props} />);

    fireEvent.change(getByLabelText("Imię", { exact: false }), {
      target: { value: "Adam" },
    });
  ...
  it("should render date in correct format", async () => {
    ...
    fireEvent.change(getByLabelText("Data urodzenia", { exact: false }), {
      target: { value: new Date("October 13, 2020 00:00:00") },
    });

    expect(
      new Date(getByLabelText("Data urodzenia", { exact: false }).value)
    ).toEqual(new Date(2020, 9, 13, 0, 0, 0));
  });
});
