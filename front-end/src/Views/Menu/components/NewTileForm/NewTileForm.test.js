...
describe("NewTileForm", () => {
    let photoFail, categoryFail, postFail, show, setShow, user, appendTile, tileData, alertC;
    let props;
    beforeAll(() => {
        //ColorPicker.mockImplementation(() => <div/>);
        Tile.mockImplementation((title, color, destination, imageUrl, showImage) => (
            <div>{title} {color} {destination} {imageUrl} {showImage}</div>
        ));
        global.fetch = jest.fn().mockImplementation((input, init) => {
            return new Promise((resolve, reject) => {
                if (photoFail || postFail || categoryFail) {
                    resolve({status: 500, json: () => Promise.resolve({message: "Błąd serwera."})})
                } else if (init === "GET") {
                    resolve({status: 200, json: () => Promise.resolve([{id: 1, category: "cat", title: "t"}])})
                } else {
                    switch(input) {
                        ...
                        case proxy.menu + "tile/":
                            if (init.method === "POST") {
                                resolve({status: 201, json: () => Promise.resolve({id: 123})});
                            } else {
