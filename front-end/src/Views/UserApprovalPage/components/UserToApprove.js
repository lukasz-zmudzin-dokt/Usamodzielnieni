...
const UserToApprove = ({ user, activeUser }) => {
    ...
    if(rejectConfirmed)
        rejectUser(context.token, user.id);
    /*if(acceptConfirmed)
    ...
    if(message) {
        ...
        return (
            <Card.Body>
                {DeletionModal(showRejectModal, setShowReject, setRejectConfirmed, "Czy na pewno chcesz odrzucić tego użytkownika?", "Odrzuć", "Anuluj")}
                <ListGroup variant="flush">
                    ...
                    <ListGroup.Item>
                        <Row className="justify-content-center">
                            <Button onClick={e => approveUser(e, context.token, user.id)} variant="success" id="accept">Akceptuj</Button>
                            <Button onClick={e => handleOnClick(e)} variant="danger" className="ml-3" id="reject">Odrzuć</Button>
                        </Row>
                    </ListGroup.Item>
                </ListGroup>
            </Card.Body>
