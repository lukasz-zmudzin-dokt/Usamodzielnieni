...
describe("UserList", () => {
    ...
    it('should render alert error on failfetch', async () => {
        ...
        expect(getByText("Ładowanie listy użytkowników...", {exact: false})).toBeInTheDocument();
    ...
    it('should render alert with no users', async () => {
        ...
        await waitForElement(() => getByText("Brak użytkowników", {exact: false}));
        expect(getByText("Brak użytkowników", {exact: false})).toBeInTheDocument();
    });
});